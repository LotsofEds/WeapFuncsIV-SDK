        ...
        public static void Tick()
        {
            ...
            if (!IS_CHAR_GETTING_UP(Main.PlayerHandle))
            {
                ...
                if (Main.SawnOffYeet)
                {
                    ...
                    {
                        if (Main.frameTime >= 0.02f)
                            timeBeforeShot = Main.frameTime;
                        else
                            timeBeforeShot = 0.02f;
                        if (IS_CHAR_PLAYING_ANIM(Main.PlayerHandle, "gun@sawnoff", "fire") && IS_CHAR_SHOOTING(Main.PlayerHandle))
                        {
                            GET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, "gun@sawnoff", "fire", out AnimPointer);
                            SET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, "gun@sawnoff", "fire", (AnimPointer - timeBeforeShot));
                        }
                        else if (IS_CHAR_PLAYING_ANIM(Main.PlayerHandle, "gun@sawnoff", "fire_crouch") && IS_CHAR_SHOOTING(Main.PlayerHandle))
                        {
                            GET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, "gun@sawnoff", "fire_crouch", out AnimPointer);
                            SET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, "gun@sawnoff", "fire_crouch", (AnimPointer - timeBeforeShot));
                        }
                    }
                }

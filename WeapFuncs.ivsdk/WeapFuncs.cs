...
namespace WeapFuncs.ivsdk
{
    internal class WeapFuncs
    {
        ...
        {
            string weaponString = settings.GetValue("INCLUDED WEAPONS", "Burst Fire Weapons", "");
            BurstWeaps.Clear();
            ...
            string rifleString = settings.GetValue("INCLUDED WEAPONS", "Burst Fire Rifles", "");
            BurstRifles.Clear();
            ...
            string pistolString = settings.GetValue("INCLUDED WEAPONS", "Burst Fire Pistols", "");
            BurstPistols.Clear();
                ...
                if (Main.SelectFire)
                {
                    foreach (eWeaponType weaponType in BurstWeaps)
                    {
                        if (Main.currWeap == (int)weaponType)
                        {
                            ...
                            if (NativeControls.IsGameKeyPressed(0, GameKey.Aim) && NativeControls.IsGameKeyPressed(0, GameKey.Action) && !NativeControls.IsGameKeyPressed(0, GameKey.Attack) && !hasPressedButton)
                            {
                                if (fireType < 2)
                                    fireType += 1;
                                else
                            ...
                            else if (!NativeControls.IsGameKeyPressed(0, GameKey.Action) && hasPressedButton)
                                ...
                                if (NativeControls.IsGameKeyPressed(0, GameKey.Attack))
                                {
                                    ...
                                    if ((lastAmmo - Main.pAmmo) == NumOfBullets)
                                    {
                                        foreach (eWeaponType rifleType in BurstRifles)
                                        {
                                            ...
                                            {
                                                SET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, pWeapAnim, "fire", 0.91f);
                                                SET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, pWeapAnim, "fire_alt", 0.91f);
                                                SET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, pWeapAnim, "fire_up", 0.91f);
                                                SET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, pWeapAnim, "fire_down", 0.91f);
                                                SET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, pWeapAnim, "fire_crouch", 0.91f);
                                                SET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, pWeapAnim, "fire_crouch_alt", 0.91f);
                                                ...
                                                SET_CHAR_ANIM_CURRENT_TIME(Main.PlayerHandle, "cover_r_high_corner", pBFAnim, 0.81f);
